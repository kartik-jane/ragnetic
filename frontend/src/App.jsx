...
export default function App() {
  ...
  const checkAuth = async () => {
    try {
      ...
      const userRes = await fetch(`${API_URL}/check-auth`, {
        credentials: "include",
      });
      if (userRes.ok) {
      ...
      const adminRes = await fetch(`${API_URL}/admin/check-auth`, {
        credentials: "include",
      });
      if (adminRes.ok) {
      ...
      const managementRes = await fetch(`${API_URL}/check-auth`, {
        credentials: "include",
      });
      if (managementRes.ok) {
  ...
  const handleFileSelect = async (e) => {
    ...
    for (const file of files) {
      ...
      try {
        const res = await fetch(`${API_URL}/upload-chat-file`, {
          method: "POST",
          credentials: "include",
          body: formData,
        });
  ...
  if (!isAuthenticated && !isManagement) {
    if (showAdminLogin) {
      return (
        <AdminLogin
          ...
          onBackClick={() => setShowAdminLogin(false)}
        />
      );
    }
    if (showManagementLogin) {
      return (
        <ManagementLogin
          onManagementLoginSuccess={handleManagementLoginSuccess}
          onBackClick={() => setShowManagementLogin(false)}
        />
      );
    }
    if (showRegister) {
    ...
    return (
      <Login
        ...
        onManagementClick={() => setShowManagementLogin(true)}
      />
    );
  }
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            ...
            : conversations.map((conv) => (
                <div
                  key={conv.id}
                  onClick={() =>
                    editingConvId !== conv.id && loadConversation(conv.id)
                  }
                ...
                >
                  ...
                  <div className="flex gap-1 ml-2 flex-shrink-0">
                    ...
                    : <>
                        ...
                        <button
                          onClick={(e) =>
                            showDeleteConfirmation(conv.id, conv.title, e)
                          }
              ...
              <div className="flex items-center justify-center h-full">
                <div className="text-center space-y-4 max-w-md">
                  ...
                  <p className="text-gray-400 text-sm">
                    Ask questions about your documents or search the web for the
                    latest information
                  </p>
                </div>
              </div>
